...
app.prepare()
  .then(() => {
    ...
    express.get('/webcam/stream', (req, res) => {
      const urlData = getURLData(req);
      const url = `http://${urlData.address}:${config.MJPEG_PORT}`
      activeStreams[url] = activeStreams[url] || new MjpegProxy(url)
      // Proxy MJPEG stream from darknet to avoid freezing issues
      return activeStreams[url].proxyRequest(req, res)
    });
    ...
    */
    express.get('/recording/:id/counter/csv', (req, res) => {
      DBManager.getCounterHistoryOfRecording(req.params.id).then((counterData) => {
        ...
        res.csv(data, false, {
          'Content-disposition': `attachment; filename=counterData-${counterData.dateStart.toISOString().split("T")[0]}-${req.params.id}.csv`
        });
      });
    })
